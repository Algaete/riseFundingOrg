...
export function registerProjectEnrichmentTests(checkAccessibility: (page: Page) => Promise<void>) {
  for (const width of [320, 1024]) {
    test(`proyecto enriquecido guarda y reabre versión ES/EN a ${width}px`, async ({ page }) => {
      ...
      await page.route(`**/api/v1/organizations/${workspaceOrganizationId}/projects/${workspaceProjectId}`, async route => {
        if (route.request().method() === 'PUT') {
          ...
          expect(input.enrichment).toMatchObject({ problem: 'Problema Ñandú', beneficiaryCount: 1200,
            latitude: -31.5, longitude: -63, locationVisibility: 0, seekingConsortium: true,
            background: { additionalInformation: 'Antecedente Ñandú' },
      ...
      await page.getByLabel(/Ubicación visible al público/).selectOption('2')
      await page.getByRole('button', { name: 'Acercar', exact: true }).click()
      await page.getByRole('button', { name: 'Mover al oeste', exact: true }).click()
      await page.getByRole('button', { name: 'Mover al sur', exact: true }).click()
      await page.getByRole('button', { name: 'Usar el centro del mapa', exact: true }).click()
      await expect(page.getByText(/^Ubicación seleccionada\./)).toBeVisible()
      // Keep the chosen point private, as in the original save/reopen regression.
      await page.getByLabel(/Ubicación visible al público/).selectOption('0')
      await page.getByLabel(/Otros antecedentes relevantes/).fill('Antecedente Ñandú')
      ...
      await expect(page.getByRole('button', { name: 'Remove map point', exact: true })).toBeVisible()
      ...
      await expect(page.getByLabel(/Latitude|Longitude/)).toHaveCount(0)
      await expect(page.getByRole('button', { name: 'Remove map point', exact: true })).toBeVisible()
      await expect(page.getByLabel(/Other relevant background/)).toHaveValue('Antecedente Ñandú')
