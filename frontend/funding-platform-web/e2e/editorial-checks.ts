...
export function registerEditorialTests(accessibility: (page: Page) => Promise<void>) {
  ...
  for (const width of [320, 1024]) {
    ...
    test(`I18N05C funding editor retains IDs money dates and dirty state at ${width}px`, async ({ page }) => {
      ...
      await expect(page.getByLabel('Closing time', { exact: true })).toHaveValue('23:59')
      await expect(page.getByLabel('Closing time', { exact: true })).toHaveAttribute('type', 'time')
      await expect(page.getByLabel('Closing time', { exact: true })).toHaveAttribute('step', '60')
      await expect(page.getByLabel('Closing date', { exact: true })).toHaveValue('2027-01-31')
      await expect(page.getByRole('combobox', { name: /^Closing time zone/ })).toHaveValue('America/Santiago')
