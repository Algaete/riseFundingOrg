...
export function registerOrganizationFundingLanguageTests(checkAccessibility: (page: Page) => Promise<void>) {
  for (const width of [320, 1024]) {
    ...
    test(`detalle interno ES/EN mantiene condiciones y destino a ${width}px`, async ({ page }) => {
      ...
      await expect(page.getByText('Fuente vinculada Ñandú', { exact: true })).toBeVisible()
      await expect(page.getByRole('link', { name: 'Fuente vinculada Ñandú', exact: true })).toHaveCount(0)
      await expect(page.locator('a[href="' + organizationOpportunity.sources[0].sourceUrl + '"]')).toHaveCount(0)
      await expect(page.getByRole('main')).not.toHaveAttribute('lang', 'es')
      await checkAccessibility(page)
      await fits(page)
      expect(writes).toHaveLength(0)
