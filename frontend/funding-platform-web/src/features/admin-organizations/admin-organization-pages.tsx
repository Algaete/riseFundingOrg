...
import { OrganizationVerificationBadge, OrganizationVerificationPanel } from './organization-verification-panel'
...
export function AdminOrganizationsWorkspacePage() {
  ...
  const verificationText = searchParams.get('verificationStatus')
  const verificationStatus = verificationText !== null && /^[0-2]$/.test(verificationText)
    ? Number(verificationText) as OrganizationVerificationStatus : undefined
  ...
  const organizations = useQuery({
    queryKey: ['admin-organizations', query, profileStatus, verificationStatus, isActive, page, pageSize],
    queryFn: ({ signal }) => adminOrganizationsApi.list({
      q: query || undefined, profileStatus, verificationStatus, isActive, page, pageSize,
    }, signal),
  ...
  return <div className="min-w-0 space-y-6">
    ...
    <Card><CardContent className="grid gap-3 p-4 sm:grid-cols-2 2xl:grid-cols-[minmax(16rem,1fr)_auto_auto_auto]">
      ...
      <label className="grid gap-1 text-xs font-semibold">{i18n.t('adminOrganizations.verificationFilter')}<select className={selectClass} onChange={event => replace('verificationStatus', event.target.value)} value={verificationStatus ?? ''}><option value="">{i18n.t('operations.all')}</option>{verificationNames.map((label, value) => <option key={label} value={value}>{i18n.t(label)}</option>)}</select></label>
