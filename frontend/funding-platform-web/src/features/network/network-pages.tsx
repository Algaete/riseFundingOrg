...
export function NetworkWorkspacePage() {
  ...
  return <div className="space-y-8">
    ...
    <section className="space-y-4"><div><h2 className="text-2xl font-bold">{t('network.directory')}</h2><p className="text-sm text-muted-foreground">{t('network.directoryHelp')}</p></div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">{directory.data?.items.map((item) => <Card key={item.id}><CardContent className="space-y-4 p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-bold">{item.name}</h3><p className="text-sm text-muted-foreground" lang="es">{item.organizationType.name} · {item.homeCountry.name}</p></div><Building2 className="size-5 text-primary" /></div><p className="line-clamp-3 text-sm" lang={item.description ? 'es' : undefined}>{item.description ?? t('network.noDescription')}</p><div className="flex flex-wrap gap-1">{item.categories.slice(0, 4).map((category) => <span className="rounded-full bg-muted px-2 py-1 text-xs" key={category.id} lang="es">{category.name}</span>)}</div><p className="text-sm">{t('network.projectCount', { count: item.visibleProjectCount })}</p><div className="flex flex-wrap gap-2"><Button asChild size="sm" variant="outline"><Link to={`/marketplace/organizations/${item.id}`}>{t('network.viewProfile')}</Link></Button>{item.connectionState === 'none' && isAdmin && item.allowsRequests && <Button onClick={() => { setSelected(item); setFeedback(null); idempotencyKey.current = null }} size="sm"><Handshake className="size-4" />{t('network.connect')}</Button>}{item.connectionState === 'none' && !item.allowsRequests && <span className="rounded-full bg-muted px-3 py-1 text-xs">{t('network.notReceiving')}</span>}{item.connectionState !== 'none' && <span className="rounded-full bg-accent px-3 py-1 text-xs font-semibold">{item.connectionState === 'connected' ? t('network.connected') : t('network.requestPending')}</span>}</div></CardContent></Card>)}</div>
    </section>
