...
function availability(opportunity: FundingOpportunityListItem) {
  ...
  if (openDate && openDate > today) return i18n.t('fundingCatalog.upcoming')
  if (opportunity.deadlineType === 2) return i18n.t('fundingCatalog.open')
  if (opportunity.deadlinePrecision === 2) {
    if (!opportunity.closeAtUtc) return i18n.t('fundingCatalog.noDeadline')
    const closeAtUtc = new Date(opportunity.closeAtUtc)
    if (!Number.isNaN(closeAtUtc.getTime())) {
      return closeAtUtc.getTime() <= now.getTime() ? i18n.t('fundingCatalog.closed') : i18n.t('fundingCatalog.open')
    }
    return i18n.t('fundingCatalog.noDeadline')
  }
  if (closeDate && closeDate < today) return i18n.t('fundingCatalog.closed')
  return closeDate ? i18n.t('fundingCatalog.open') : i18n.t('fundingCatalog.noDeadline')
}
...
function SourceAttribution({ opportunity }: { opportunity: FundingOpportunityListItem }) {
  const { t } = useTranslation()
  const grantsNotice = !opportunity.sourceAttribution?.trim()
  ...
  const attribution = opportunity.sourceAttribution?.trim()
    || (opportunity.sourceName.trim().toLocaleLowerCase() === 'grants.gov'
      ...
      : t('fundingCatalog.sourceHelp', { name: opportunity.sourceName }))
      ...
      <CardHeader className="space-y-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="text-xs font-medium text-muted-foreground">
            {t('fundingCatalog.quality', { score: qualityScore(opportunity.dataQualityScore) })}
          </span>
        </div>
        <div>
      ...
      <CardContent className="flex flex-1 flex-col gap-5">
        ...
        <dl className="mt-auto grid gap-3 text-sm">
          <div>
            <dt className="flex items-start gap-3 font-semibold">
              <CalendarDays aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-primary" />{t('fundingCatalog.close')}</dt>
            <dd className="ml-7 text-muted-foreground">{formatDate(opportunity.closeDate)}</dd>
          </div>
          <div>
            <dt className="flex items-start gap-3 font-semibold">
              <CircleDollarSign aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-primary" />{t('fundingCatalog.funding')}</dt>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <Button asChild className={action ? undefined : 'sm:col-span-2'} size="sm">
            <Link to={detailHref}>{t('fundingCatalog.viewDetail')}</Link>
          </Button>
          {action}
        </div>
...
function errorMessage(error: unknown) {
  return discoveryErrorMessage(error, 'fundingCatalog.loadHelp')
}
...
function ApplicationExitInterstitial({ url }: { url: string }) {
  ...
  return (
    <>
      <Button onClick={() => setOpen(true)} type="button">{t('fundingCatalog.apply')}<ExternalLink className="size-4" />
      </Button>
      {open && (
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold" id="external-site-title">{t('fundingCatalog.exitTitle')}</h2>
                  <p className="mt-2 text-sm leading-6 text-muted-foreground" id="external-site-description">{t('fundingCatalog.exitHelp')}</p>
                </div>
              </div>
              <Button
                aria-label={t('fundingCatalog.closeConfirmation')}
                ref={closeButton}
            ...
            <div className="mt-5 rounded-xl border bg-muted p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{t('fundingCatalog.destination')}</p>
              <p className="mt-1 break-all font-mono text-sm font-semibold">{destination.hostname}</p>
            </div>
            <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <Button onClick={() => setOpen(false)} type="button" variant="outline">{t('fundingCatalog.back')}</Button>
              <Button asChild className="h-auto min-h-10 min-w-0 whitespace-normal break-all">
                <a
                  ...
                  target="_blank"
                >
                  {t('fundingCatalog.continueTo', { hostname: destination.hostname })} <ExternalLink className="size-4" />
                </a>
              </Button>
            </div>
          </div>
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:py-12">
      <div className="rounded-2xl border bg-card p-6 shadow-sm sm:p-8">
        ...
        <form className="mt-6 flex max-w-2xl flex-col gap-2 sm:flex-row" onSubmit={submitSearch}>
          <label className="sr-only" htmlFor="funding-search">{t('fundingCatalog.searchLabel')}</label>
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={t('fundingCatalog.searchPlaceholder')}
              value={draftQuery}
            />
          </div>
          <Button type="submit">{t('fundingCatalog.search')}</Button>
        </form>
      </div>
      ...
      {opportunities.isPending && (
        <div className="space-y-4" role="status">
          <p className="flex items-center gap-2 text-sm text-muted-foreground">
            <LoaderCircle className="size-4 animate-spin" />{t('fundingCatalog.loading')}</p>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3" aria-label={t('fundingCatalog.loadingGrid')}>
      ...
      {opportunities.isError && (
        <Card className="border-destructive/40">
          <CardContent className="p-6" role="alert">
            <p className="font-semibold">{t('fundingCatalog.loadFailed')}</p>
            <p className="mt-1 text-sm text-muted-foreground">{errorMessage(opportunities.error)}</p>
            <Button className="mt-4" onClick={() => void opportunities.refetch()} variant="outline">{t('fundingCatalog.retry')}</Button>
          </CardContent>
        </Card>
      )}
        ...
        <section aria-busy={opportunities.isFetching} className="space-y-5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-sm text-muted-foreground">
              <span className="font-bold text-foreground">{opportunities.data.totalCount.toLocaleString(workspaceLocale())}</span>{' '}
              {t('fundingCatalog.published', { count: opportunities.data.totalCount })}
            </p>
            <p className="flex items-center gap-2 text-xs text-muted-foreground">
              {opportunities.isFetching
                ? <><LoaderCircle className="size-4 animate-spin" />{t('fundingCatalog.refreshing')}</>
                : <><ShieldCheck className="size-4 text-primary" />{t('fundingCatalog.traceable')}</>}
            </p>
          </div>
          {opportunities.data.items.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                <h2 className="font-semibold">{t('fundingCatalog.emptyTitle')}</h2>
                <p className="mt-2 text-sm text-muted-foreground">{t('fundingCatalog.emptyHelp')}</p>
              </CardContent>
            </Card>
          ) : (
          ...
          {opportunities.data.totalCount > opportunities.data.pageSize && (
            <nav aria-label={t('fundingCatalog.pagination')} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-card p-3 sm:justify-end sm:gap-4">
              <Button
                ...
                variant="outline"
              >
                <ChevronLeft className="size-4" />{t('fundingCatalog.previous')}</Button>
              <p className="text-sm text-muted-foreground">
                {t('fundingCatalog.page', { page: opportunities.data.pageNumber, total: lastPage })}
              </p>
              <Button
                ...
                type="button"
                variant="outline"
              >{t('fundingCatalog.next')}<ChevronRight className="size-4" />
              </Button>
            </nav>
          )}
        </section>
...
export function FundingOpportunityDetailView({
  item,
  backTo,
  backLabel = i18n.t('fundingCatalog.backOpportunities'),
  action,
  additionalDetails,
}: {
  ...
  const { t, i18n: language } = useTranslation()
  return (
    <div className="space-y-6" lang={language.resolvedLanguage}>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
          ...
          <div className="space-y-6 p-6 sm:p-8">
            <header className="space-y-4 border-b pb-6">
              <div className="flex flex-wrap items-center gap-2 text-xs font-bold">
                ...
                <span className="text-muted-foreground">{t('fundingCatalog.quality', { score: qualityScore(item.dataQualityScore) })}</span>
                {item.externalId && <span className="text-muted-foreground">{t('fundingCatalog.externalId', { id: item.externalId })}</span>}
              </div>
            ...
            <section>
              <h2 className="text-xl font-bold">{t('fundingCatalog.detailDescription')}</h2>
              <p className="mt-3 whitespace-pre-line text-base leading-8 text-muted-foreground">
                {item.description ?? item.summary ?? i18n.t('fundingCatalog.noDescription')}
              </p>
            </section>

            {item.eligibilityDescription && (
              <section className="rounded-xl bg-muted p-5">
                <h2 className="text-xl font-bold">{t('fundingCatalog.eligibility')}</h2>
            ...
            {item.requirements && (
              <section>
                <h2 className="text-xl font-bold">{t('fundingCatalog.requirements')}</h2>
            ...
            {item.objectives && (
              <section>
                <h2 className="text-xl font-bold">{t('fundingCatalog.objectives')}</h2>
            ...
            {item.funders.length > 0 && (
              <section>
                <h2 className="text-xl font-bold">{t('fundingCatalog.funders')}</h2>
                  ...
                  {item.funders.map((funder) => (
                    <li className="rounded-lg border px-4 py-3 text-sm" key={`${funder.funderId}-${funder.role}`}>
                      ...
                      <span className="ml-2 text-xs text-muted-foreground">
                        {funder.role === 1 ? i18n.t('fundingCatalog.primary') : funder.role === 2 ? i18n.t('fundingCatalog.cofunder') : i18n.t('fundingCatalog.administrator')}
                      </span>
                    </li>
                  ))}
                </ul>
        ...
        <aside className="space-y-4">
          {action}
          <Card>
            <CardHeader><CardTitle>{t('fundingCatalog.applicationDetails')}</CardTitle></CardHeader>
            <CardContent>
              <dl className="space-y-4 text-sm">
                <div><dt className="font-semibold">{t('fundingCatalog.opening')}</dt><dd className="mt-1 text-muted-foreground">{formatDate(item.openDate)}</dd></div>
                <div><dt className="font-semibold">{t('fundingCatalog.close')}</dt><dd className="mt-1 text-muted-foreground">{formatDate(item.closeDate)}</dd></div>
                <div><dt className="font-semibold">{t('fundingCatalog.amount')}</dt><dd className="mt-1 text-muted-foreground">{formatAmount(item)}</dd></div>
                <div><dt className="font-semibold">{t('fundingCatalog.cofunding')}</dt><dd className="mt-1 text-muted-foreground">{item.requiresCofunding === null ? i18n.t('fundingCatalog.notReported') : item.requiresCofunding ? i18n.t('fundingCatalog.yes') : i18n.t('fundingCatalog.no')}</dd></div>
              </dl>
          ...
          <Card>
            <CardHeader><CardTitle>{t('fundingCatalog.verification')}</CardTitle></CardHeader>
            <CardContent className="space-y-3 text-sm">
              <p><span className="text-muted-foreground">{t('fundingCatalog.lastVerified')}</span><strong>{formatDateTime(item.lastVerifiedAtUtc)}</strong></p>
              <p><span className="text-muted-foreground">{t('fundingCatalog.dataQuality')}</span><strong>{qualityScore(item.dataQualityScore)}/100</strong></p>
              <p className="text-xs leading-5 text-muted-foreground">{t('fundingCatalog.qualityHelp')}</p>
            </CardContent>
          </Card>
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6" role="status">
        <p className="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
          <LoaderCircle className="size-4 animate-spin" />{t('fundingCatalog.loadingDetail')}</p>
      ...
      <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
        <Card>
          <CardContent className="space-y-4 p-8" role="alert">
            <h1 className="text-2xl font-bold">{t('fundingCatalog.detailFailed')}</h1>
            <p className="text-sm text-muted-foreground">{errorMessage(opportunity.error)}</p>
            <Button asChild variant="outline">
              <Link to="/funding">{t('fundingCatalog.backCatalog')}</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
