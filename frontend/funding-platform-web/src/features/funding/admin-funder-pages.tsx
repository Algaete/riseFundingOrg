...
function AdminFunderForm({ funder, onDirtyChange }: { funder?: AdminFunderDetail; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  if (catalogs.isPending) {
    return <p className="flex items-center gap-2" role="status"><LoaderCircle className="size-4 animate-spin" /> {i18n.t('adminFunders.loadingCountries')}</p>
  }
        ...
        <CardContent>
          <fieldset className="grid gap-5 disabled:opacity-65" disabled={locked || save.isPending}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field error={form.formState.errors.websiteUrl?.message} label={i18n.t('adminFunders.website')}>
            ...
            <Field error={form.formState.errors.aliasesText?.message} label={i18n.t('adminFunders.aliases')}>
              <textarea {...form.register('aliasesText')} className={textareaClass} placeholder={i18n.t('adminFunders.aliasesPlaceholder')} />
            </Field>
            {locked && (
              <p className="rounded-lg bg-muted p-3 text-sm">
                {i18n.t('editorial.lockedContent', { status: i18n.t(publicationStatusKeys[funder!.publicationStatus]).toLocaleLowerCase() })}
              </p>
            )}
          </fieldset>
          {save.isError && (
            <div className="mt-5 rounded-lg bg-destructive/10 p-3 text-sm text-foreground" role="alert">
      ...
      {!locked && (
        <div className="flex justify-end">
          <Button disabled={save.isPending || (Boolean(funder) && !form.formState.isDirty)} type="submit">
            ...
            {funder ? i18n.t('editorial.save') : i18n.t('adminFunders.create')}
          </Button>
        </div>
      )}
    </form>
        ...
        <CardContent className="p-4">
          <form className="grid gap-3 sm:grid-cols-[1fr_14rem_auto]" onSubmit={search}>
            <label className="relative">
              <span className="sr-only">{i18n.t('adminFunders.search')}</span>
              <Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
              <Input className="pl-9" onChange={(event) => setDraftQuery(event.target.value)} placeholder={i18n.t('adminFunders.searchPlaceholder')} value={draftQuery} />
            </label>
            <label>
              <span className="sr-only">{i18n.t('editorial.filterStatus')}</span>
              <select
                className={inputClass}
                onChange={(event) => { setStatus(event.target.value === '' ? '' : Number(event.target.value) as PublicationStatus); setPage(1) }}
                value={status}
              >
                <option value="">{i18n.t('editorial.allStatuses')}</option>
                {Object.entries(publicationStatusKeys).map(([value, label]) => <option key={value} value={value}>{i18n.t(label)}</option>)}
              </select>
            </label>
            <Button type="submit">{i18n.t('editorial.search')}</Button>
          </form>
        </CardContent>
      </Card>

      {funders.isPending && <p className="flex items-center gap-2" role="status"><LoaderCircle className="size-5 animate-spin" /> {i18n.t('adminFunders.loading')}</p>}
      {funders.isError && (
        <Card><CardContent className="space-y-3 p-6" role="alert"><p className="text-foreground">{adminErrorMessage(funders.error)}</p><Button onClick={() => void funders.refetch()} variant="outline">{i18n.t('editorial.retry')}</Button></CardContent></Card>
      )}
      {funders.data && (
        <section aria-busy={funders.isFetching} className="space-y-4">
          <p className="text-sm text-muted-foreground">{i18n.t('adminFunders.count', { count: funders.data.totalCount })}</p>
          {funders.data.items.length === 0 ? (
            <Card><CardContent className="p-10 text-center"><Building2 className="mx-auto size-10 text-primary" /><h2 className="mt-3 text-xl font-bold">{i18n.t('adminFunders.empty')}</h2><p className="mt-2 text-muted-foreground">{i18n.t('adminFunders.emptyHelp')}</p></CardContent></Card>
          ) : (
                ...
                <Card key={funder.funderId}>
                  <CardContent className="space-y-4 p-5">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div><h2 className="text-xl font-bold">{funder.name}</h2><p className="mt-1 text-xs text-muted-foreground">{i18n.t('editorial.versionUpdated', { version: funder.contentVersion, date: formatAdminDate(funder.updatedAtUtc) })}</p></div>
          ...
          {funders.data.totalCount > funders.data.pageSize && (
            <nav aria-label={i18n.t('adminFunders.pagination')} className="flex flex-wrap items-center justify-end gap-3">
              <Button disabled={page <= 1 || funders.isFetching} onClick={() => setPage((value) => value - 1)} variant="outline"><ChevronLeft className="size-4" />{i18n.t('editorial.previous')}</Button>
              <p className="text-sm">{i18n.t('editorial.page', { page: funders.data.page, total: lastPage })}</p>
              <Button disabled={page >= lastPage || funders.isFetching} onClick={() => setPage((value) => value + 1)} variant="outline">{i18n.t('editorial.next')}<ChevronRight className="size-4" /></Button>
            </nav>
          )}
        </section>
      )}
