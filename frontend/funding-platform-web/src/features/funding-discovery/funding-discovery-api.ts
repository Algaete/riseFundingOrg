...
export interface FundingExplorerPage { items: { id: string; title: string; slug: string; summary: string | null; sourceName: string; sourceUrl: string; lastVerifiedAtUtc: string | null;
  minimumAmount: number | null; maximumAmount: number | null; currency: string | null; closeDate: string | null; classification: DiscoveryClassification | null;
  localization?: FundingLocalization | null }[]; totalCount: number; page: number; pageSize: number }
export const fundingDiscoveryApi = {
  ...
  search: (params: URLSearchParams, signal?: AbortSignal, locale?: FundingLanguage) => {
    const localized = new URLSearchParams(params)
    localized.delete('locale')
    if (locale) localized.set('locale', locale)
    return apiClient.get<FundingExplorerPage>(`funding-discovery?${localized}`, { signal, ...(locale ? { cache: 'no-store' as const } : {}) })
  },
