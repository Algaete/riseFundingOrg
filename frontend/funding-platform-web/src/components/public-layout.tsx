...
import { ContactLink } from '@/features/engagement/engagement-ui'
...
export function PublicLayout() {
  ...
  const links = [
    ...
    { to: '/stories', key: 'stories', extra: true },
    { to: '/services', key: 'services', extra: true },
    { to: '/contact', key: 'contact', extra: true },
  ] as const
  ...
  return (
    <div className={`min-h-screen ${isHome ? 'reference-home' : ''}`}>
      <header className="sticky top-0 z-20 border-b bg-background/95 backdrop-blur" onKeyDown={event => {
        ...
        <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-3 px-4 py-2 sm:px-6">
          ...
          <nav className="hidden items-center gap-1 lg:flex" aria-label={t('navigation.main')}>
            {links.map(link => <Button key={link.key} variant="ghost" asChild className={`px-2 text-[13px] font-medium ${'extra' in link ? 'hidden' : ''}`}>
              <Link to={link.to}>{t(`navigation.${link.key}`)}</Link>
            </Button>)}
            <Button variant="ghost" className="px-2" aria-expanded={menuOpen} aria-controls="public-explore-menu" onClick={toggleMenu}>{t('navigation.more')}<ChevronDown className="size-3" aria-hidden="true" /></Button>
      ...
      <main lang={translatedDiscovery || /^\/stories\/[^/]+\/?$/.test(pathname) || translatedPaths.has(pathname.toLowerCase().replace(/\/+$/, '') || '/') ? undefined : 'es'}>
        <Outlet />
      </main>
      {isHome ? <footer className="home-footer"><strong>{t('home.footer.title')}</strong><span>{t('home.footer.subtitle')}</span><ContactLink /></footer>
        : <footer className="space-y-3 border-t px-4 py-8 text-center text-sm text-muted-foreground"><p>{t('layout.footer')}</p><ContactLink /></footer>}
    </div>
  )
}
