...
trabajo pendiente de otros bloques. El release de países se aísla y vuelve a validar
...
- Smoke SQL `048_world_country_catalog_smoke.sql`: comprueba las 249 identidades y
  ...
  borrador y con rollback. **Preparado y parseado, no ejecutado contra SQL real.**

## Estado previo a la publicación

Al terminar el desarrollo, esta corrección estaba local, sin commit, push,
despliegue ni cambios en Azure. El propietario autorizó su publicación en dev.
Para verla en dev hay que aplicar la migración 048 con el migrador existente y
publicar el frontend. Un despliegue sólo del frontend seguiría mostrando Chile.
Validar el smoke en el entorno autorizado y comprobar selección/guardado allí;
recargar la página para vaciar la caché de catálogos de la sesión anterior.
No reprovisionar infraestructura ni cambiar el SKU o la autopausa de SQL.
