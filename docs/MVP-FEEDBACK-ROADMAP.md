...
## Nuevo feedback — 2026-09-21

Validación posterior de estos bloques: **preflight052–059 aprobado en Azure dev**
(59 smokes, 23 checks de mapa, 13 de traducciones), rollback y limpieza verificados.
Regresión local:1139 unitarias .NET. Esto supera los pendientes de preflight de los
cortes históricos siguientes, pero no despliega ni activa funciones. Falta release
seleccionado, QA y los módulos no implementados. Ver
[resultado y reparaciones](runbooks/feedback-sql-preflight-2026-09-21.md).

FundsforNGOs está pausado por petición del usuario. Primer bloque de usabilidad
implementado localmente: USD en nuevos presupuestos, búsqueda de países, ejemplos
de indicadores, selección de punto en mapa, eliminación de enlaces públicos
adicionales a fuentes, cierre en hora local/minutos con lista de zonas y ayuda para
carga manual. **Sin desplegar.**

Segundo bloque local (2A): persistencia y revisión de traducciones ES/EN con versiones,
editor administrativo y ficha localizada. Migración 053/flags pendientes de activar.
No genera traducciones automáticamente ni tradujo datos de Azure.
Ver [traducciones: alcance y pendientes](FUNDING-CONTENT-TRANSLATIONS.md).

Tercer bloque local: «Otros (especificar)» y cuatro campos opcionales de antecedentes
de proyectos, con validación, versión editorial y lectura pública controlada.
Migraciones 054/055 pendientes de preflight y despliegue; no incluye archivos.
Ver [contrato del bloque 3](FUNDING-CATEGORIES-AND-PROJECT-BACKGROUND.md).

Cuarto bloque local (4A): biblioteca administrable de portadas temáticas, con selección
en editor, versión editorial y visualización pública/de organización. Migración 056
pendiente de preflight/QA/despliegue. Sin uploads ni servicios cloud nuevos.
Ver [contrato de portadas](FUNDING-COVER-LIBRARY.md).

Ampliación local 2B-1: títulos/resúmenes traducidos en catálogo, organización,
favoritos, explorador y búsqueda unificada; solo revisión vigente, un lote SQL por
página y caché separada por idioma. SQL057 y preflight/despliegue pendientes.
Ver [listados localizados y límites](FUNDING-LIST-TRANSLATIONS.md).

Ampliación local 2B-2: búsqueda literal del original y títulos/resúmenes revisados
vigentes ES/EN, sin duplicados y antes de contar/paginar. Flags conservados en false,
SQL058 pendiente de preflight/despliegue. Sin IA por visita ni infraestructura nueva.
Ver [búsqueda bilingüe y límites](FUNDING-BILINGUAL-SEARCH.md).

No está completo el nuevo encargo: faltan generación automática y traducciones iniciales,
fotografías/logos propios, adjuntos/material de suscripción y descubrimiento abierto
de convocatorias con revisión editorial.
Ver [alcance, módulos y pendientes actuales](FEEDBACK-UX-2026-09-21.md).
Los cortes que siguen son evidencia histórica, no el estado del nuevo feedback.
