/* 055 transactional project background smoke. Synthetic fixtures are rolled back.
...
/* Pure validator checks do not write any fixture or invoke external services. */
IF dbo.FundingPlatform_fn_ProjectBackgroundIsValid(NULL) <> 1
   OR dbo.FundingPlatform_fn_ProjectBackgroundIsValid(N'{}') <> 1
   OR dbo.FundingPlatform_fn_ProjectBackgroundIsValid(N'{"additionalInformation":null,"previousResults":"Resultado"}') <> 1
   OR dbo.FundingPlatform_fn_ProjectBackgroundIsValid(N'[]') <> 0
   OR dbo.FundingPlatform_fn_ProjectBackgroundIsValid(N'not json') <> 0
   OR dbo.FundingPlatform_fn_ProjectBackgroundIsValid(N'{"unknown":"x"}') <> 0
   OR dbo.FundingPlatform_fn_ProjectBackgroundIsValid(N'{"additionalInformation":1}') <> 0
   OR dbo.FundingPlatform_fn_ProjectBackgroundIsValid(N'{"additionalInformation":"a","additionalInformation":"b"}') <> 0
    THROW 56080, N'Background shape validation failed.', 1;
DECLARE @Limit NVARCHAR(MAX) = N'{"previousResults":"' + REPLICATE(CONVERT(NVARCHAR(MAX), N'ñ'), 3000) + N'"}';
IF dbo.FundingPlatform_fn_ProjectBackgroundIsValid(@Limit) <> 1
    THROW 56081, N'Background rejects 3000 characters.', 1;
SET @Limit = N'{"previousResults":"' + REPLICATE(CONVERT(NVARCHAR(MAX), N'ñ'), 3001) + N'"}';
IF dbo.FundingPlatform_fn_ProjectBackgroundIsValid(@Limit) <> 0
    THROW 56082, N'Background accepts oversized text.', 1;
IF @UpdateDefinition NOT LIKE N'%Legacy update cannot erase project background%'
    THROW 56083, N'Legacy preservation guard is missing.', 1;

DECLARE @InitialTransactionCount INT = @@TRANCOUNT;
IF @InitialTransactionCount = 0 BEGIN TRANSACTION;
ELSE SAVE TRANSACTION FP_Smoke055;

BEGIN TRY
    DECLARE @Fixture UNIQUEIDENTIFIER = NEWID();
    DECLARE @UserPublicId UNIQUEIDENTIFIER = NEWID();
    DECLARE @Email NVARCHAR(320) = N'project-background-' +
    ...
    DECLARE @Slug NVARCHAR(180) = N'project-background-' +
    ...
    IF (SELECT COUNT_BIG(1) FROM @PublicRows) <> 1 OR EXISTS
        (SELECT 1 FROM @PublicRows WHERE JSON_VALUE(EnrichmentJson, '$.latitude') IS NOT NULL
            ...
            OR COALESCE(JSON_VALUE(EnrichmentJson, '$.background.additionalInformation'), N'') <> N'Contexto público')
    ...
    IF (SELECT COUNT_BIG(1) FROM @PublicRows) <> 1 OR EXISTS
        (SELECT 1 FROM @PublicRows
         WHERE COALESCE(TRY_CONVERT(DECIMAL(10,2), JSON_VALUE(EnrichmentJson, '$.latitude')), 999) <> -33.46
            ...
            OR COALESCE(JSON_VALUE(EnrichmentJson, '$.background.previousResults'), N'') <> N'240 hogares')
    ...
    /* background {} explicitly clears the section; omission cannot erase stored background. */
    DECLARE @RowVersion2 BINARY(8) = (SELECT RowVersion FROM dbo.FundingPlatform_Projects WHERE Id = @ProjectId);
    DECLARE @ClearSnapshot NVARCHAR(MAX) = N'{"title":"Enrichment cleared","projectStage":null,"sustainableDevelopmentGoalIds":[],"enrichment":{"background":{}}}';
    ...
    INSERT INTO @Created EXEC dbo.FundingPlatform_usp_Project_Update
        ...
        @SustainableDevelopmentGoalIdsJson = N'[]', @EnrichmentJson = N'{"background":{}}';
    IF NOT EXISTS
        (SELECT 1 FROM dbo.FundingPlatform_Projects
         WHERE Id = @ProjectId AND ProjectVersion = 3 AND EnrichmentJson = N'{"background":{}}')
       OR NOT EXISTS
        (SELECT 1 FROM dbo.FundingPlatform_ProjectVersions
