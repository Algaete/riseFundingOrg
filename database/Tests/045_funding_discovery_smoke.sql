...
ELSE SAVE TRANSACTION FP_Smoke045;
BEGIN TRY
    ...
    UPDATE dbo.FundingPlatform_Users SET TwoFactorEnabled = 1 WHERE Id = @OwnerId;
    INSERT dbo.FundingPlatform_UserRoles(UserId, RoleId)
       SELECT @OwnerId, Id FROM dbo.FundingPlatform_Roles WHERE NormalizedName = N'ADMIN';
    DECLARE @Classification NVARCHAR(MAX) = N'{"funderKind":3,"requiresConsortium":null,"requiresInternationalPartner":false,"evidenceUrl":"https://example.invalid/fund"}';
    DECLARE @Json TABLE(Json NVARCHAR(MAX));
    DECLARE @ParentVersion INT = (SELECT ContentVersion FROM dbo.FundingPlatform_FundingOpportunities WHERE PublicId = @Opportunity);
    SET @Key = HASHBYTES('SHA2_256', N'classify-opportunity');
    INSERT @Json EXEC dbo.FundingPlatform_usp_FundingDiscovery_Review @Owner, @Opportunity, @ParentVersion, @Classification, NULL, @Key, @Hash;
    IF COALESCE((SELECT JSON_VALUE(Json, '$.wasReplay') FROM @Json), N'') <> N'false'
       THROW 55750, N'Classification was not created.', 1;
    DELETE @Json;
    INSERT @Json EXEC dbo.FundingPlatform_usp_FundingDiscovery_Review @Owner, @Opportunity, @ParentVersion, @Classification, NULL, @Key, @Hash;
    IF COALESCE((SELECT JSON_VALUE(Json, '$.wasReplay') FROM @Json), N'') <> N'true'
       THROW 55751, N'Classification replay failed.', 1;
    DELETE @Json;
    INSERT @Json EXEC dbo.FundingPlatform_usp_FundingDiscovery_AdminGet @Owner, @Opportunity;
    IF COALESCE((SELECT JSON_VALUE(Json, '$.data.funderKind') FROM @Json), N'') <> N'3'
       THROW 55752, N'Classification read lost data.', 1;
    DELETE @Json;
    EXEC dbo.FundingPlatform_usp_FundingDiscovery_Search N'{"page":1,"pageSize":20,"onlyOpen":true,"funderKind":3,"requiresInternationalPartner":false}';
    IF EXISTS(SELECT 1 FROM dbo.FundingPlatform_ifn_FundingOpportunityPublicReady() r
       INNER JOIN dbo.FundingPlatform_FundingOpportunities p ON p.Id = r.FundingOpportunityId WHERE p.PublicId = @Opportunity)
       THROW 55753, N'Classification bypassed draft publication.', 1;
    UPDATE dbo.FundingPlatform_FundingOpportunities SET ContentVersion = ContentVersion + 1 WHERE PublicId = @Opportunity;
    IF EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingDiscovery m
       INNER JOIN dbo.FundingPlatform_FundingOpportunities p ON p.Id = m.FundingOpportunityId AND p.ContentVersion = m.ContentVersion
       WHERE p.PublicId = @Opportunity)
       THROW 55754, N'Stale classification remained current.', 1;
    IF @InitialTransactionCount = 0 ROLLBACK TRANSACTION;
    ELSE ROLLBACK TRANSACTION FP_Smoke045;
END TRY
BEGIN CATCH
    IF @InitialTransactionCount = 0 AND @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
    ELSE IF XACT_STATE() = 1 ROLLBACK TRANSACTION FP_Smoke045;
    THROW;
END CATCH;
