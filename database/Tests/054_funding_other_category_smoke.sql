/* Other-category fixtures and ownership regression; rollback only. Run on disposable/dev SQL after 054.
...
ELSE SAVE TRANSACTION FP_Smoke054;
BEGIN TRY
    ...
    DECLARE @OpportunityResult TABLE(Succeeded BIT, Code NVARCHAR(50), FundingOpportunityPublicId UNIQUEIDENTIFIER,
        ContentVersion INT, PublicationStatus TINYINT, RowVersion BINARY(8), WasReplay BIT);
    DECLARE @Snapshot NVARCHAR(MAX) = N'{"otherCategoryDescription":"Economía circular"}';
    ...
    INSERT INTO @OpportunityResult EXEC dbo.FundingPlatform_usp_FundingOpportunity_Create
        ...
        @OtherCategoryDescription = N'Economía circular',
    ...
    IF NOT EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingOpportunities
                  WHERE Id = @OpportunityId AND OtherCategoryDescription = N'Economía circular' AND PublicationStatus = 0)
    ...
    SET @Snapshot = N'{"otherCategoryDescription":null}';
    ...
    INSERT INTO @OpportunityResult EXEC dbo.FundingPlatform_usp_FundingOpportunity_Update
        ...
        @SnapshotJson = @Snapshot, @ContentHash = @ContentHash, @OtherCategoryDescription = NULL,
        @CountryIds = @Countries, @RegionIds = @Regions, @CategoryIds = @Categories,
        @BeneficiaryTypeIds = @Beneficiaries, @ProjectTypeIds = @Types, @FunderLinksJson = @Links,
    ...
    IF NOT EXISTS(SELECT 1 FROM @OpportunityResult WHERE Succeeded = 1 AND ContentVersion = 2 AND PublicationStatus = 0)
       OR NOT EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingOpportunities
                     WHERE Id = @OpportunityId AND OtherCategoryDescription IS NULL)
       OR NOT EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingOpportunityVersions
    ...
    ELSE ROLLBACK TRANSACTION FP_Smoke054;
END TRY
BEGIN CATCH
    IF @InitialTransactionCount = 0 AND @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
    ELSE IF XACT_STATE() = 1 ROLLBACK TRANSACTION FP_Smoke054;
    THROW;
END CATCH;
