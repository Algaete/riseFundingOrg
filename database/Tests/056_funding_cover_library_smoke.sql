/* Cover library fixtures and ownership regression; rollback only. Run on disposable/dev SQL after 056.
...
ELSE SAVE TRANSACTION FP_Smoke056;
BEGIN TRY
    ...
    DECLARE @OpportunityResult TABLE(Succeeded BIT, Code NVARCHAR(50), FundingOpportunityPublicId UNIQUEIDENTIFIER,
        ContentVersion INT, PublicationStatus TINYINT, RowVersion BINARY(8), WasReplay BIT);
    DECLARE @Snapshot NVARCHAR(MAX) = N'{"otherCategoryDescription":"Economía circular","coverKey":"nature-v1"}';
    DECLARE @ContentHash BINARY(32) = HASHBYTES('SHA2_256', @Snapshot);
    ...
    INSERT INTO @OpportunityResult EXEC dbo.FundingPlatform_usp_FundingOpportunity_Create
        ...
        @SourceItemKeyHash = @Hash, @SourceUrl = N'https://example.invalid/fund', @SnapshotJson = @Snapshot, @ContentHash = @ContentHash,
        @OtherCategoryDescription = N'Economía circular', @CoverKey = N'nature-v1',
    ...
    DECLARE @OpportunityId BIGINT = (SELECT Id FROM dbo.FundingPlatform_FundingOpportunities WHERE PublicId = @Opportunity);
    IF NOT EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingOpportunities
                  WHERE Id = @OpportunityId AND OtherCategoryDescription = N'Economía circular' AND PublicationStatus = 0 AND CoverKey = N'nature-v1')
       OR NOT EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingOpportunityVersions
                     WHERE FundingOpportunityId = @OpportunityId AND ContentVersion = 1 AND SnapshotJson = @Snapshot)
        THROW 56070, N'Other category did not persist with its draft snapshot.', 1;
    EXEC dbo.FundingPlatform_usp_FundingOpportunity_Admin_Get @AdminUserPublicId = @Owner,
        @FundingOpportunityPublicId = @Opportunity, @OwnerWorkspace = 1;
    DECLARE @OpportunityVersion BINARY(8) = (SELECT RowVersion FROM @OpportunityResult);
    /* Legacy callers may omit a cover only before an editorial selection exists. */
    DELETE FROM @OpportunityResult;
    DECLARE @LegacySnapshot NVARCHAR(MAX) = N'{"otherCategoryDescription":"Economía circular"}';
    DECLARE @LegacyHash BINARY(32) = HASHBYTES('SHA2_256', @LegacySnapshot);
    DECLARE @LegacyKey BINARY(32) = HASHBYTES('SHA2_256', N'legacy-cover-omission');
    INSERT INTO @OpportunityResult EXEC dbo.FundingPlatform_usp_FundingOpportunity_Update
        @AdminUserPublicId = @Owner, @FundingOpportunityPublicId = @Opportunity, @ExpectedRowVersion = @OpportunityVersion,
        @Title = N'Legacy opportunity', @SponsorName = @Slug, @Summary = N'Synthetic draft',
        @AmountStatus = 2, @DeadlineType = 2, @DeadlinePrecision = 0, @GeographicScope = 2,
        @RemoteApplication = 0, @DataQualityScore = 0, @FundingSourceId = @SourceId,
        @SourceItemKeyHash = @Hash, @SourceUrl = N'https://example.invalid/fund',
        @SnapshotJson = @LegacySnapshot, @ContentHash = @LegacyHash, @OtherCategoryDescription = N'Economía circular',
        @CountryIds = @Countries, @RegionIds = @Regions, @CategoryIds = @Categories,
        @BeneficiaryTypeIds = @Beneficiaries, @ProjectTypeIds = @Types, @FunderLinksJson = @Links,
        @IdempotencyKeyHash = @LegacyKey, @RequestHash = @LegacyHash, @OwnerWorkspace = 1;
    IF NOT EXISTS(SELECT 1 FROM @OpportunityResult WHERE Succeeded = 0 AND Code = N'cover-selection-required')
       OR NOT EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingOpportunities
                     WHERE Id = @OpportunityId AND CoverKey = N'nature-v1' AND RowVersion = @OpportunityVersion)
       OR (SELECT COUNT_BIG(1) FROM dbo.FundingPlatform_FundingOpportunityVersions WHERE FundingOpportunityId = @OpportunityId) <> 1
        THROW 56091, N'Legacy update erased a cover or its snapshot.', 1;

    DELETE FROM @OpportunityResult;
    DELETE FROM @Categories;
    INSERT INTO @Categories VALUES(1);
    SET @Snapshot = N'{"otherCategoryDescription":null,"coverKey":"auto"}';
    SET @ContentHash = HASHBYTES('SHA2_256', @Snapshot);
    SET @Key = HASHBYTES('SHA2_256', N'other-category-clear');
    INSERT INTO @OpportunityResult EXEC dbo.FundingPlatform_usp_FundingOpportunity_Update
        @AdminUserPublicId = @Owner, @FundingOpportunityPublicId = @Opportunity, @ExpectedRowVersion = @OpportunityVersion,
        @Title = N'Owner opportunity', @SponsorName = @Slug, @Summary = N'Synthetic draft', @Description = N'Other removed',
        @AmountStatus = 2, @DeadlineType = 2, @DeadlinePrecision = 0, @GeographicScope = 2,
        @RemoteApplication = 0, @DataQualityScore = 0, @FundingSourceId = @SourceId,
        @SourceItemKeyHash = @Hash, @SourceUrl = N'https://example.invalid/fund',
        @SnapshotJson = @Snapshot, @ContentHash = @ContentHash, @OtherCategoryDescription = NULL, @CoverKey = N'auto',
        @CountryIds = @Countries, @RegionIds = @Regions, @CategoryIds = @Categories,
        @BeneficiaryTypeIds = @Beneficiaries, @ProjectTypeIds = @Types, @FunderLinksJson = @Links,
        @IdempotencyKeyHash = @Key, @RequestHash = @ContentHash, @OwnerWorkspace = 1;
    IF NOT EXISTS(SELECT 1 FROM @OpportunityResult WHERE Succeeded = 1 AND ContentVersion = 2 AND PublicationStatus = 0)
       OR NOT EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingOpportunities
                     WHERE Id = @OpportunityId AND OtherCategoryDescription IS NULL AND CoverKey = N'auto')
       OR NOT EXISTS(SELECT 1 FROM dbo.FundingPlatform_FundingOpportunityVersions
                     WHERE FundingOpportunityId = @OpportunityId AND ContentVersion = 2 AND SnapshotJson = @Snapshot)
        THROW 56071, N'Other-category removal must clear text and preserve versioned draft workflow.', 1;
    DELETE FROM @OpportunityResult;
    INSERT INTO @OpportunityResult EXEC dbo.FundingPlatform_usp_FundingOpportunity_Update
        @AdminUserPublicId = @Owner, @FundingOpportunityPublicId = @Opportunity, @ExpectedRowVersion = @OpportunityVersion,
        @Title = N'Owner opportunity', @SponsorName = @Slug, @Summary = N'Synthetic draft', @Description = N'Other removed',
        ...
        @BeneficiaryTypeIds = @Beneficiaries, @ProjectTypeIds = @Types, @FunderLinksJson = @Links,
        @IdempotencyKeyHash = @Key, @RequestHash = @ContentHash, @OwnerWorkspace = 1;
    IF NOT EXISTS(SELECT 1 FROM @OpportunityResult WHERE Succeeded = 1 AND WasReplay = 1 AND ContentVersion = 2)
       OR (SELECT COUNT_BIG(1) FROM dbo.FundingPlatform_FundingOpportunityVersions WHERE FundingOpportunityId = @OpportunityId) <> 2
        THROW 56072, N'Other-category replay created a duplicate version.', 1;
    BEGIN TRY
    ...
    ELSE ROLLBACK TRANSACTION FP_Smoke056;
END TRY
BEGIN CATCH
    IF @InitialTransactionCount = 0 AND @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
    ELSE IF XACT_STATE() = 1 ROLLBACK TRANSACTION FP_Smoke056;
    THROW;
END CATCH;
