/* Optional background text reuses project versions, publication review and private/public reads.
   No attachments, URL fetching, services or additional runtime grants.
   Full procedure definitions preserve migration 040 guards; nested background omission is fail-closed in SQL. */
SET NOCOUNT ON;
SET XACT_ABORT ON;
IF EXISTS(SELECT 1 FROM sys.check_constraints WHERE parent_object_id = OBJECT_ID(N'dbo.FundingPlatform_Projects')
    AND name = N'FundingPlatform_CK_Projects_EnrichmentJson')
    ALTER TABLE dbo.FundingPlatform_Projects DROP CONSTRAINT FundingPlatform_CK_Projects_EnrichmentJson;
ALTER TABLE dbo.FundingPlatform_Projects WITH CHECK ADD CONSTRAINT FundingPlatform_CK_Projects_EnrichmentJson
    CHECK(EnrichmentJson IS NULL OR (ISJSON(EnrichmentJson, OBJECT) = 1 AND DATALENGTH(EnrichmentJson) <= 400000));
GO

CREATE OR ALTER FUNCTION dbo.FundingPlatform_fn_ProjectBackgroundIsValid(@Json NVARCHAR(MAX))
RETURNS BIT
AS
BEGIN
    IF @Json IS NULL RETURN 1;
    IF ISJSON(@Json, OBJECT) <> 1 OR DATALENGTH(@Json) > 160000 RETURN 0;
    IF EXISTS(SELECT 1 FROM OPENJSON(@Json)
        GROUP BY [key] COLLATE Latin1_General_100_BIN2 HAVING COUNT(*) > 1) RETURN 0;
    IF EXISTS(SELECT 1 FROM OPENJSON(@Json)
        WHERE [key] COLLATE Latin1_General_100_BIN2 NOT IN
            (N'additionalInformation',N'technicalInformation',N'existingPartnerships',N'previousResults')
           OR [type] NOT IN(0,1) OR DATALENGTH([value]) > 6000) RETURN 0;
    RETURN 1;
END;
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @EnrichmentJson IS NOT NULL AND
           (ISJSON(@EnrichmentJson, OBJECT) <> 1 OR DATALENGTH(@EnrichmentJson) > 400000)
               ...
               (
                   ...
                   WHERE [key] COLLATE Latin1_General_100_BIN2 NOT IN
                     (N'problem', N'solution', N'beneficiaryCount', N'locality', N'latitude',
                      ...
                      N'soughtProfessionals', N'seekingConsortium', N'background')
                      ...
                      OR ([key] = N'background' AND [type] NOT IN (0, 5))
               )
                THROW 51410, N'Project enrichment contains invalid properties.', 1;
            IF dbo.FundingPlatform_fn_ProjectBackgroundIsValid(JSON_QUERY(@EnrichmentJson, '$.background')) = 0
                THROW 51410, N'Project background must contain bounded optional text fields.', 1;
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @EnrichmentJson IS NOT NULL AND ISJSON(@EnrichmentJson, OBJECT) <> 1
            THROW 51410, N'Project enrichment must be a bounded JSON object.', 1;
        IF JSON_QUERY(@EnrichmentJson, '$.background') IS NULL AND EXISTS
            (SELECT 1 FROM dbo.FundingPlatform_Projects WHERE Id = @ProjectId AND JSON_QUERY(EnrichmentJson, '$.background') IS NOT NULL)
            THROW 51411, N'Legacy update cannot erase project background; provide the complete section or an explicit empty object.', 1;
        IF ISJSON(@SnapshotJson) <> 1 THROW 51403, N'Project snapshot must be JSON.', 1;
...
BEGIN
    ...
    SELECT projects.PublicId AS ProjectPublicId, projects.Slug, projects.Title,
           ...
           (SELECT JSON_VALUE(projects.EnrichmentJson, '$.problem') AS problem,
                   ...
                   JSON_QUERY(CASE WHEN dbo.FundingPlatform_fn_ProjectBackgroundIsValid(JSON_QUERY(projects.EnrichmentJson, '$.background')) = 1
                        THEN JSON_QUERY(projects.EnrichmentJson, '$.background') END) AS background,
