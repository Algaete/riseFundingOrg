...
{
    // SQL JSON may omit an empty collection or return null. Keep the API contract
    // non-null, including detail lookups that consume this page internally.
    public IReadOnlyList<Story> Items { get; init; } = Items ?? [];
}
