...
public sealed class ProjectAssetDefenderEventGridFunction(
    IEventGridBearerTokenValidator tokens,
    ProjectAssetDefenderEventGridService service,
    ILogger<ProjectAssetDefenderEventGridFunction> logger)
{
    [Function(nameof(ProjectAssetDefenderEventGridFunction))]
        ...
        CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation(
            "Project-asset Defender delivery finished with {OutcomeCode} for asset {ProjectAssetId}.",
            result.Code,
            result.ProjectAssetId);

        return result.Outcome switch
        {
            ProjectAssetDefenderEventGridOutcome.ValidationHandshake => await ResponseAsync(
                request,
                HttpStatusCode.OK,
                new { validationResponse = result.ValidationCode },
                cancellationToken),
            ProjectAssetDefenderEventGridOutcome.Applied => await ResponseAsync(
                request,
                HttpStatusCode.OK,
                new { code = result.Code },
                cancellationToken),
            ProjectAssetDefenderEventGridOutcome.Rejected => await ResponseAsync(
                request,
